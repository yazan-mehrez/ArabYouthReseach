...
class MysqliDb
{
     ...
     * @var array
     */
    protected $_joinAnd = array(); // Create the empty 0 index
    /**
    ...
    public function operation($tableName, $numRows = null, $operation , $column , $columns = '*')
    {
        if (empty($columns)) {
            $columns = '*';
        }
        ...
        $this->_query = 'SELECT '. $operation . '(' . $column . ')' . implode(' ', $this->_queryOptions) . ' ' .
            $column . " FROM " . $this->_tableName;
        $stmt = $this->_buildQuery($numRows);
     ...
     * long-running js, or to reconnect timed out connections (if php.ini has
